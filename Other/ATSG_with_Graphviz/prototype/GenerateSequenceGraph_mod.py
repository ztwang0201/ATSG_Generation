...
def set_child_components_list_for_branches(step_index, ob_nodes, parent, parent_components_list, components_of_each_branch):
    # 新規のparentのbranch
    if parent not in parent_components_list:
...
def expand_assembly_unit(ob_nodes):
    ...
    #             ob_list.append(item[0])
    #
    #     for fastener in fastener_list:
    #         if fastener in ob_list:
    #             if len(ob_list) > 2:
    #                 is_expansion = True
    #                 break
    #         else:
    ...
    #
    #     else:
    #         new_ob_nodes.append(ob_nodes[step_index])
    ...
    for step_index in range(len(yolo_data)):
        ...
        for item in ob_nodes[step_index]:
            ...
            if object_name == parent and object_index == 1:
                ...
                children, child_components_list = set_child_components_list(step_index, ob_nodes, parent, child_components_list)
    ...
    for step_index in range(len(ob_nodes)):
        ...
        for item in ob_nodes[step_index]:
            ...
            if object_base_name not in pre_step_child_components_list:
                ...
                in_node_base_name_list.append(set_object_base_name(object_name, object_index))  # object_name[object_index]
            ...
            parent = set_parent_component(components_hierarchy, ob_nodes, step_index, object_name, hierarchy_data)
            if object_name == parent and object_index == 1:
                # children, child_components_list = set_child_components_list_mod(step_index, ob_nodes, parent, child_components_list, parent_components_list)
                children, child_components_list, components_of_each_branch = set_child_components_list_for_branches(step_index, ob_nodes, parent,
                ...
                out_node_base_name_list.append(set_object_base_name(object_name, object_index))  # object_name[object_index]
                    ...
                    if removable_node in body_str:
                        ...
                        if step_of_removable_node[in_node_index] > step_of_inheriable_node[out_node_index]:
                            if inherited_node not in inherited_node_list:
                                # print(removable_node_list)
                                # print(inheritable_node_list)
                                # print(dg.body.index(body_str), body_str)
                                # print(dg.body.index(body_str) + 1, dg.body[dg.body.index(body_str) + 1])
                                # print('------------------------------------------------------')
                                dg.body.remove(dg.body[dg.body.index(body_str) + 1])
                                dg.body.remove(body_str)

                                dst_motion_node = dst_motion_node_list[in_node_index]
                                # print(removable_node_list, len(removable_node_list))
                                # print(dst_motion_node_list, len(dst_motion_node_list))
                                # print('------------------------------------------------------')
...
d_exp_seq_graph = Digraph(format="svg")
